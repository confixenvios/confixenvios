-- Corrigir o trigger para não fazer chamada HTTP direta (que estava causando erro)
-- Apenas loggar que o webhook precisa ser disparado
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status != NEW.status AND NEW.status IN ('PAID', 'PAYMENT_CONFIRMED', 'LABEL_GENERATED')) THEN
    ...
    -- Log que webhook está sendo processado automaticamente (sem chamada HTTP problemática)
    INSERT INTO public.webhook_logs (
      event_type,
      shipment_id,
      payload,
      ...
      jsonb_build_object(
        ...
        'note', 'Webhook data prepared - will be dispatched by edge function call'
      )
    );
    
    -- Log adicional para debug
    RAISE NOTICE 'Webhook data prepared automatically for shipment % with complete data including pricing table: %', NEW.tracking_code, CASE WHEN pricing_table_data IS NOT NULL THEN NEW.pricing_table_name ELSE 'NONE' END;
  END IF;
