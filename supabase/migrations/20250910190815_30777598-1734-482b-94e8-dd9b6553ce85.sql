-- Corrigir erro de tipos na função manual de dispatch
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Se não tem dados criptografados, tentar buscar do quote_data
    IF shipment_record.quote_data IS NOT NULL THEN
      SELECT 
        shipment_record.quote_data->'addressData'->'sender'->>'document' as document,
        shipment_record.quote_data->'addressData'->'sender'->>'phone' as phone,
        shipment_record.quote_data->'addressData'->'sender'->>'email' as email
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    IF shipment_record.quote_data IS NOT NULL THEN
      SELECT 
        shipment_record.quote_data->'addressData'->'recipient'->>'document' as document,
        shipment_record.quote_data->'addressData'->'recipient'->>'phone' as phone,
        shipment_record.quote_data->'addressData'->'recipient'->>'email' as email
  ...
  -- Extrair dados da declaração de conteúdo se existirem (corrigindo tipos)
  ...
  IF shipment_record.document_type = 'declaracao_conteudo' AND shipment_record.quote_data IS NOT NULL THEN
    declaration_data := jsonb_build_object(
      'document_number', COALESCE((shipment_record.quote_data->'fiscalData'->>'declarationNumber')::text, (shipment_record.quote_data->'documentData'->'declarationData'->>'documentNumber')::text, '01'),
      'issue_date', COALESCE((shipment_record.quote_data->'fiscalData'->>'issueDate')::text, (shipment_record.quote_data->'documentData'->'declarationData'->>'issueDate')::text, date_trunc('day', now())::text),
      'delivery_forecast', COALESCE((shipment_record.quote_data->'fiscalData'->>'deliveryForecast')::text, (shipment_record.quote_data->'documentData'->'declarationData'->>'deliveryForecast')::text, NULL),
      'total_value', COALESCE((shipment_record.quote_data->'fiscalData'->'totalValue')::numeric, (shipment_record.quote_data->'documentData'->'declarationData'->'totalValue')::numeric, (shipment_record.quote_data->'merchandiseDetails'->>'totalValue')::numeric, (shipment_record.quote_data->>'totalMerchandiseValue')::numeric, 0),
      'content_description', COALESCE((shipment_record.quote_data->'fiscalData'->>'contentDescription')::text, (shipment_record.quote_data->'documentData'->>'merchandiseDescription')::text, (shipment_record.quote_data->>'merchandiseDescription')::text, 'Mercadoria')
    );
  END IF;
  
  -- Montar payload completo para envio via webhook
  webhook_payload := jsonb_build_object(
    ...
    'shipment_data', jsonb_build_object(
      ...
      'sender_address', jsonb_build_object(
        ...
        -- Dados pessoais (criptografados ou do quote_data)
      ...
      'nfe_data', CASE 
        WHEN shipment_record.document_type = 'nota_fiscal_eletronica' THEN 
          jsonb_build_object(
            'access_key', COALESCE((shipment_record.quote_data->'fiscalData'->>'nfeAccessKey')::text, (shipment_record.quote_data->'documentData'->>'nfeKey')::text)
          )
        ELSE null
      END,
      ...
      'shipping_details', jsonb_build_object(
        ...
        'estimated_price', COALESCE((shipment_record.quote_data->'shippingQuote'->>'economicPrice')::numeric, (shipment_record.quote_data->'shippingQuote'->>'expressPrice')::numeric, 0),
        'estimated_days', COALESCE((shipment_record.quote_data->'shippingQuote'->>'economicDays')::integer, (shipment_record.quote_data->'shippingQuote'->>'expressDays')::integer, 0),
        'merchandise_value', COALESCE((shipment_record.quote_data->>'totalMerchandiseValue')::numeric, (shipment_record.quote_data->'merchandiseDetails'->>'totalValue')::numeric, 0),
        'merchandise_description', COALESCE((shipment_record.quote_data->>'merchandiseDescription')::text, declaration_data->>'content_description', 'Mercadoria')
      )
    ),
    'timestamp', now()
  );
  ...
  RETURN jsonb_build_object(
    ...
    'webhook_payload_size', length(webhook_payload::text)
  );
END;
$function$;