-- Adicionar coluna motorista_id na tabela b2b_shipments
ALTER TABLE public.b2b_shipments 
ADD COLUMN IF NOT EXISTS motorista_id uuid REFERENCES auth.users(id);

-- Recriar a função accept_shipment para incluir motorista_id no B2B
...
BEGIN
  ...
  -- Tentar atualizar shipment normal
  UPDATE shipments
  SET 
  ...
  -- Tentar atualizar B2B shipment (agora com motorista_id)
