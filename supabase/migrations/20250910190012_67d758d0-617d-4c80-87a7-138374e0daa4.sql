-- Corrigir função de trigger para webhook de remessa
-- Garantir que todos os dados necessários para CTE sejam enviados
-- Primeiro dropar o trigger, depois a função, depois recriar ambos

-- 1. Dropar o trigger primeiro
DROP TRIGGER IF EXISTS on_shipment_created ON public.shipments;

-- 2. Dropar a função
DROP FUNCTION IF EXISTS public.trigger_shipment_webhook();

-- 3. Recriar a função corrigida com coleta de dados completos
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status != NEW.status AND NEW.status IN ('PAID', 'PAYMENT_CONFIRMED', 'LABEL_GENERATED')) THEN
    ...
    -- Buscar dados pessoais criptografados do remetente
    ...
    -- Buscar dados pessoais criptografados do destinatário
    ...
    webhook_payload := jsonb_build_object(
      ...
      'shipment_data', jsonb_build_object(
        ...
        'sender_address', jsonb_build_object(
          ...
          'document', sender_personal_data.document,
          'phone', sender_personal_data.phone,
          'email', sender_personal_data.email
        ),
        ...
        'recipient_address', jsonb_build_object(
          ...
          -- Dados pessoais descriptografados
          'document', recipient_personal_data.document,
          'phone', recipient_personal_data.phone,
          'email', recipient_personal_data.email
        ),
