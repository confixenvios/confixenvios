-- Atualizar função get_motorista_shipments para retornar todos os campos necessários
CREATE OR REPLACE FUNCTION public.get_motorista_shipments(motorista_uuid uuid)
 RETURNS TABLE(id uuid, tracking_code text, status text, created_at timestamp with time zone, updated_at timestamp with time zone, weight numeric, length numeric, width numeric, height numeric, format text, selected_option text, pickup_option text, quote_data jsonb, payment_data jsonb, cte_key text, sender_address_id uuid, recipient_address_id uuid, sender_address jsonb, recipient_address jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Retornar remessas do motorista com dados completos dos endereços
  RETURN QUERY
  SELECT 
    s.id,
    s.tracking_code,
