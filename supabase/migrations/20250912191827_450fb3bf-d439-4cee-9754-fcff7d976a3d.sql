-- LIMPEZA DE PRODUÇÃO: Remover todos os elementos de teste e demo
...
USING (
  ...
  AND (
    -- Apenas admins ou sistema podem acessar
    has_role(auth.uid(), 'admin'::app_role) OR
    -- Ou se for uma inserção válida com motorista ativo
    (
      TG_OP = 'INSERT' AND 
      (metadata->>'motorista_id') IS NOT NULL
    )
  )
)
WITH CHECK (
