-- Atualizar o trigger de webhook para incluir informações da tabela de preços
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status != NEW.status AND NEW.status IN ('PAID', 'PAYMENT_CONFIRMED', 'LABEL_GENERATED')) THEN
    ...
    RAISE NOTICE 'Webhook triggered for shipment % with complete data including pricing table info', NEW.tracking_code;
  END IF;
