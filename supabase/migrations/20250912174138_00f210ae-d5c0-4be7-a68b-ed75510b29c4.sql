-- Atualizar o trigger para incluir dados completos da tabela de preços no webhook
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status != NEW.status AND NEW.status IN ('PAID', 'PAYMENT_CONFIRMED', 'LABEL_GENERATED')) THEN
    ...
    INSERT INTO public.webhook_logs (
      ...
      202,
      jsonb_build_object(
        'status', 'webhook_triggered_automatic_with_pricing_table',
    ...
    BEGIN
      ...
      INSERT INTO public.webhook_logs (
        ...
        NEW.id::text,
        jsonb_build_object('function_called', 'shipment-webhook-dispatch', 'pricing_table_included', pricing_table_data IS NOT NULL),
        200,
        webhook_response
      );
    ...
    RAISE NOTICE 'Webhook triggered automatically for shipment % with complete data including pricing table: %', NEW.tracking_code, CASE WHEN pricing_table_data IS NOT NULL THEN NEW.pricing_table_name ELSE 'NONE' END;
  END IF;
