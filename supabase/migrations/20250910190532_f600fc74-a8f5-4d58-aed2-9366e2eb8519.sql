-- Atualizar a função para funcionar mesmo sem dados pessoais criptografados
...
BEGIN
  ...
  -- Buscar integração ativa para envio do webhook
  SELECT webhook_url, secret_key INTO integration_record
  ...
  INSERT INTO public.webhook_logs (
    ...
    jsonb_build_object(
      ...
      'integration_found', integration_record IS NOT NULL,
      'webhook_url', CASE WHEN integration_record IS NOT NULL THEN integration_record.webhook_url ELSE 'NO_INTEGRATION' END
    )
  );
  ...
  RETURN jsonb_build_object(
    ...
    'payload_preview', jsonb_build_object(
      ...
      'document_type', shipment_record.document_type,
      'integration_configured', integration_record IS NOT NULL
    ),
