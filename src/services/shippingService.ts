...
export const calculateShippingQuote = async ({
  ...
  try {
    ...
    // Primeiro, tentar buscar cotação das tabelas de preços cadastradas
    ...
    // Fallback para o sistema antigo se não houver tabelas ou erro
    return await calculateLegacyShippingQuote({ destinyCep, weight, quantity });
  } catch (error) {
    console.error('Erro ao calcular frete:', error);
    
    // Tentar fallback para o sistema antigo em caso de erro
    try {
      return await calculateLegacyShippingQuote({ destinyCep, weight, quantity });
    } catch (fallbackError) {
