      ...
      <div className="flex-1 py-4 px-3 overflow-y-auto">
        ...
        <Collapsible open={localOpen} onOpenChange={setLocalOpen}>
          <CollapsibleTrigger asChild>
            <button className={cn(
              ...
              isInLocal ? 'bg-primary/10 text-primary' : 'text-foreground hover:bg-slate-100'
            )}>
              <div className="flex items-center gap-3">
                <Car className="h-4 w-4" />
                <span className="text-sm font-semibold">Envios Locais</span>
              </div>
              <ChevronDown className={cn("h-4 w-4 transition-transform", localOpen && "rotate-180")} />
            </button>
          </CollapsibleTrigger>
            ...
            {localMenuItems.map((item) => (
              <NavLink
                ...
                className={cn(
                  ...
                  isActive(item.url) ? 'bg-primary text-white shadow-md' : 'text-foreground hover:bg-slate-100'
                )}
              >
        ...
        <Collapsible open={convencionalOpen} onOpenChange={setConvencionalOpen} className="mt-2">
          <CollapsibleTrigger asChild>
            <button className={cn(
              "w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg transition-all mb-1",
              isInConvencional ? 'bg-primary/10 text-primary' : 'text-foreground hover:bg-slate-100'
            )}>
              <div className="flex items-center gap-3">
                <Truck className="h-4 w-4" />
                <span className="text-sm font-semibold">Envios Nacionais</span>
              </div>
              <ChevronDown className={cn("h-4 w-4 transition-transform", convencionalOpen && "rotate-180")} />
            </button>
          </CollapsibleTrigger>
          <CollapsibleContent className="pl-4 space-y-1">
        ...
        <NavLink
          ...
          className={cn(
            ...
            isActive('/painel/relatorios') ? 'bg-primary text-white shadow-md' : 'text-foreground hover:bg-slate-100'
          )}
        >
        ...
        <NavLink
          ...
          className={cn(
            "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all mt-2",
            isActive('/painel/minha-conta') ? 'bg-primary text-white shadow-md' : 'text-foreground hover:bg-slate-100'
          )}
        >
        ...
        <NavLink
          ...
          className={cn(
            "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all mt-2",
            location.pathname.startsWith('/painel/suporte') ? 'bg-primary text-white shadow-md' : 'text-foreground hover:bg-slate-100'
          )}
        >
